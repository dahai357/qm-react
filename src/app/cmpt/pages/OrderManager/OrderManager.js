...
export default class OrderManager extends BaseCmpt {
    ...
    loadData(page,arg,noUpdateFlag) {
        var self = this;
        orderApi.orderList(page,0,arg).then(function (res) {
            ...
            if(!noUpdateFlag){
                loginApi.count().then(function (_res) {
                    UpdateMenuNum('orderCount',_res.responseContent.orderCount);
                })
            } 
        })
    }
            ...
            <React.Fragment>
...

                {
                    orders.map(function (item) {
                        return <Distribution key={item.orderId} order={item} serverTime={serverTime} showDada={showDada} />
                    })
                }
